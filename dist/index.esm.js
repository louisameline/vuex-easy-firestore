...
function pluginMutations (userState) {
    return {
        ...
        DELETE_DOC: function (state, id) {
            var _this = this;
            var s = state._conf.statePropName ? state[state._conf.statePropName] : state;
            if (state._conf.firestoreRefType.toLowerCase() !== 'collection') {
                Object.keys(s).forEach(function (key) {
                    if (!(['_conf', '_sync']).includes(key)) {
                        _this._vm.$delete(s, key);
                    }
                });
            }
            else {
        ...
        UNREGISTER_SUBMODULE: function (state, id) {
            this.unregisterModule([state._conf.moduleName, id]);
        }
    };
}
...
function convertTimestamps(originVal, targetVal) {
    if (originVal === '%convertTimestamp%') {
        ...
        // @ts-ignore
        if (isAnyObject(targetVal) && !isPlainObject(targetVal) && isFunction$1(targetVal.toDate)) {
            // @ts-ignore
 ...
 */
function pluginActions (Firebase, appVersion) {
    var _this = this;
    return {
        ...
        patchDoc: function (_a, _b) {
            ...
            state._sync.syncStack.inserts.forEach(function (newDoc, newDocIndex) {
                ...
                var patchDoc = getters.prepareForInsert({ id: newDoc.id, doc: newDoc.doc });
        ...
        insertDoc: function (_a, payload) {
            ...
            return new Promise(function (resolve, reject) {
                ...
                if (state._conf.sync.insertHookBeforeSync) {
                    // @ts-ignore
                    state._conf.sync.insertHookBeforeSync(resolve, payload.doc, _this);
                }
                else {
                    resolve();
                }
        ...
        addLocalSubmodule: function (_a, params) {
            ...
            var submoduleConfig = merge({}, state._conf.submodule);
            if (!params.id) {
        ...
        addRemoteSubmodule: function (_a, id) {
            var getters = _a.getters, state = _a.state, commit = _a.commit, dispatch = _a.dispatch;
        ...
        unregisterSubmodules: function (_a) {
            var getters = _a.getters, state = _a.state, commit = _a.commit, dispatch = _a.dispatch;
            getters.submoduleIds.forEach(function (id) {
        ...
        _a, _b) {
            ...
            return new Promise(function (resolve, reject) {
                ...
                function storeUpdateFn(_doc) {
                    if (_doc) {
                        ...
                        var edit = function () {
                            ...
                            if (metadata_1) {
                                Object.assign(_state._metadata, metadata_1);
                            }
                        };
                        switch (change) {
                            ...
                            case 'added':
                                ...
                                if (store.hasModule([state._conf.moduleName, id])) {
                                    ...
                                    dispatch('addLocalSubmodule', {
                                        id: id,
                                        state: { data: _doc, _metadata: metadata_1 }
                                    });
                                }
                                break;
                            // relevant for collections only
                ...
                var conf = getters.collectionMode ? state._conf.submodule : state._conf, syncHookFn = conf.serverChange ? conf.serverChange[change + 'Hook'] : null;
        ...
        deleteMissingProps: function (_a, doc) {
            ...
            var clearObject = function (toBeClearedObj, RefObj, path) {
                if (path === void 0) { path = []; }
                for (var key in toBeClearedObj) {
                    var pathCopy = path.slice(0);
                    pathCopy.push(key);
                    var stringPath = pathCopy.join('.');
                    if (!RefObj.hasOwnProperty(key)) {
                        var excluded = ['_conf', '_sync'];
        ...
        openDBChannel: function (_a, parameters) {
            ...
            if (isFunction$1(state._sync.unsubscribe[identifier])) {
                var streamAlreadyOpenError = "openDBChannel was already called for these clauses and pathvariables. Identifier: " + identifier;
                if (state._conf.logging) {
            ...
            function (snapshot) { return __awaiter(_this, void 0, void 0, function () {
                var docChanges, promises_1, resp;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            ...
                            docChanges = snapshot.docChanges({ includeMetadataChanges: true }), promises_1 = new Array(docChanges.length);
                            // debug messages
                            if (parameters.debug) {
                        ...
                        case 2:
                            ...
                            return [4 /*yield*/, processDocument(snapshot)];
                        case 3:
                            resp = _a.sent();
 ...
 */
function pluginGetters (Firebase, appVersion) {
    return {
        ...
        prepareForInsert: function (state, getters, rootState, rootGetters) {
            return function (item) {
                ...
                // clean up item
                var conf = getters.collectionMode
                    ? state[item.id]._conf
                    : state._conf;
                if (conf.metadata) {
 ...
 */
function iniModule (userConfig, FirebaseDependency, appVersion) {
    // prepare state._conf
...
function vuexEasyFirestore(easyFirestoreModule, _a) {
    var _b = _a === void 0 ? {
        ...
        appVersion: null
    } : _a, _c = _b.logging, logging = _c === void 0 ? false : _c, _d = _b.preventInitialDocInsertion, preventInitialDocInsertion = _d === void 0 ? false : _d, _e = _b.FirebaseDependency, FirebaseDependency = _e === void 0 ? firebase : _e, _f = _b.appVersion, appVersion = _f === void 0 ? null : _f;
